...
pod2usage(1) if ($OPT{help} || !$OPT{coord_bed} || !$OPT{super_mutants} || !$OPT{variants} || !$OPT{r_dir});
...
graph.pl -coord_bed coord_bed_file -super_mutants super_mutants_file -variants variants_from_file -r_dir R_directory

Required flags: -coord_bed -super_mutants -variants -r_dir
...
my $variants = $OPT{variants};
if ( !-e $variants ) {
	modules::Exception->throw("File $variants doesn't exist");	
}
...
open(VAR,"$variants") || modules::Exception->throw("Can't open file $variants\n");
while (<VAR>) {
	chomp;
    next unless /\S/;
    my ($chr,$coord,$count) = split("\t");
    $var_tally{"$chr:$coord"} = $count;
}
...
while (<BED>) {
	...
	my $r_super = join(" ", 'Rscript', $r_dir .'/supermut_coord.R',$workdir,$sm_out,$sm_pdf);
	my $r_variants = join(" ", 'Rscript', $r_dir .'/mut_coord.R',$workdir,$var_out,$var_pdf);
	push @r_commands, $r_super, $r_variants;
}
